...
void * create_image(void * num){
    ...
    for( int px=*((int*)num); px<width; px+=nthreads)
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
			Color color;
			color.r = scaled_color[0];
			color.g = scaled_color[1];
			color.b = scaled_color[2];

			colorArray[px][py] = color;
        }
    }
	return NULL;
}
int main( int argc, char **argv )
{
   ...
   for(int t = 0; t < nthreads; t++){
	 ids[t] = t;
	 int ret = pthread_create(&threads[t], NULL, create_image, &ids[t]);
	 if(ret){
	   fprintf(stderr, "Thread creation failed. Error code is %d\n", ret);
	   exit(-1);
	 }
   }
   ...
   for(int t = 0; t < nthreads; t++){
	 ...
	 if(ret){
	   fprintf(stderr, "Threading joining failed. Erro code is %d\n", ret);
	   exit(-1);
	 }
   }
   int i, j;
   ...
   for( i = 0; i < width; i++){
	 for( j = 0; j < height; j++){
	   printf("%.0f %.0f %.0f\n", colorArray[i][j].r, colorArray[i][j].g, colorArray[i][j].b);
	 }
	 printf("\n");
   }
   free_scene( &scene );
